    ...
    int pageSize)
        {
            ...
            if (endDate.HasValue)
            {
                // Trong AnalyticsService, endDate đã được tính là ngày cuối tháng, nên ta dùng <= là đủ.
                query = query.Where(t => t.TransactionDate <= endDate.Value);
            }

            // Đếm tổng số bản ghi và phân trang trong MỘT LỆNH BẤT ĐỒNG BỘ (FIX)
            // Thực hiện truy vấn.
            // Lệnh này vẫn cần Đếm trước khi Skip/Take.
            var totalCount = await query.CountAsync();

            // Phân trang và sắp xếp theo ngày giảm dần (Lệnh AWAIT 2)
